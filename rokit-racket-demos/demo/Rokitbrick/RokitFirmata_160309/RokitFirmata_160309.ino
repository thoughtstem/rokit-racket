...
void loop() {
  ...
  {
    for(int j = 0; j < 20; j++)  { if(bufferBytes[j][5] == READY) {isReady = true; follow = j; break;} }
  }  
...
void serialEvent() {
  ...
  while (Serial.available()) {
      ...
      if((inByte == 0xFF)&&(exInByte == 0xFF)) { isReceiving = true; exInByte = 0;}
      else {  exInByte = inByte; }       
   }
}
