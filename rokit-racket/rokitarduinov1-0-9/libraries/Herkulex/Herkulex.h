...
#define DATA_MOVE  	 120	// max 24 servos <---- change this for more servos!
#define TIME_OUT     5   	// msec (between bytes timeout)
...
public:
	...
	//////////////////////////////
	void  writeRegistryRAM_2byte(int servoID, int address, int writeByte);
	void  writeRegistryEEP_2byte(int servoID, int address, int writeByte);	
	void  set_KP(int servoID, int writeByte);
	//////////////////////////////
	
	void  maximumVin(int servoID, int voltage);
	void  minimumVin(int servoID, int voltage);
	
	int readRegistryRam(int servoID, int address, int writeByte);
