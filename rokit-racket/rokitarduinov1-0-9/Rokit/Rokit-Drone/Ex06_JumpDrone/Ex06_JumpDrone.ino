...
void setup()  // loop에 넣지 않고 setup에 넣어야 한번만 실행
{
  Serial.begin(9600);                  // 시리얼 통신(무선통신) 개시
  SmartDroneControl.begin();         // 드론 플러그의 기능 개시
  delay(1000);

  SmartDroneControl.ResetYaw();       //0.5초가 ResetYaw 필수
  delay(500);
...
void loop()
{
  ...
  int analogValue3 = analogRead(A3); //pitch 전 후
  int analogValue4 = analogRead(A4); //yaw 좌 우
  int analogValue5 = analogRead(A5); //throttle 상 하
  int analogValue6 = analogRead(A6); //roll 회 전
